        ...
        let html = `
            <dialog id="dropdownRecruitMore" class="option-dropdown border-normal drop-shadow-lg">
                <ul class="nav font-bold text-gray-600 w-70 pb-0!">
                    <li>
                        <button type="button" onclick="navigator.clipboard.writeText('http://www.jobchae.kro.kr${pageContext.request.contextPath}/recruit/view/\${recruit_no}').then(() => {
                            alert('링크가 클립보드에 복사되었습니다.');});" class="py-2"><i class="fa-solid fa-link"></i> 링크 복사
                        </button>
                    </li>
                    <%-- <li><a href="${pageContext.request.contextPath}/recruit/view/\${recruit_no}"><i class="fa-solid fa-bookmark"></i> 채용공고 삭제</a></li> --%>
                </ul>
            </dialog>`;
...
function getRecruitList(recruit_closed) {
    ...
    $.ajax({
        url: "${pageContext.request.contextPath}/api/recruit/my-save",
        data: {"start": start
            ,"end": start + len - 1
        ...
        success: function (json) {
            ...
            if(json.length > 0) {
                ...
                $.each(json, (index, item) => {
                    ...
                    html += `
                        <li>
                            <a href="${pageContext.request.contextPath}/recruit/view/\${item.recruit_no}" class="w-full text-left flex">
                ...
                if(start==1) { // 목록이 하나도 없다면
                    let html = `
                        <li>
                            ...
                            <button type="button" id="btnSearchFocus" class="button-orange mt-4 mb-8">채용공고 검색</button>
                        </li>`;
        ...
        <div class="left-side col-span-3 hidden md:block h-full relative">
            <div class="border-normal sticky">
                ...
                <ul class="recruit-nav pb-0!">
                    <li><a href="${pageContext.request.contextPath}/recruit/main/upload">게시한 채용공고</a></li>
                    <li class="recruit-nav-selected"><a href="${pageContext.request.contextPath}/recruit/main/save">지원한 채용공고</a></li>
