...
function gomemberLogin() {
    ...
    // 유효성 검사
    if ($(`input#login_member_id`).val().trim() == "") {
        alert("아이디를 입력하세요!");
        $(`input#login_member_id`).val("");
        return; // 종료
    }


    if ($(`input#login_member_passwd`).val().trim() == "") {
        alert("비밀번호를 입력하세요!");
        $(`input#login_member_passwd`).val("");
        return; // 종료
    }
    ...
    if ($(`input:checkbox[id="saveid"]`).prop("checked")) {
        ...
        localStorage.setItem("saveid", $(`input#login_member_id`).val());
...
function goRegister() { // 세션에서 얻어오는 것이 좋을 듯
    ...
    // 로그아웃을 처리해주는 페이지로 이동
    location.href = `${contextPath}/member/memberRegister`;
