...
$(document).ready(function () {
    ...
    // passwd 블러 처리
    $("input#new_member_passwd").blur((e) => {
    ...
    $("input#passwdcheck").blur((e) => {
        ...
        if ($("input#new_member_passwd").val() != $(e.target).val()) {
            // 암호와 암호확인값이 틀린 경우 
    ...
    $(`button#btn_email_auth`).click(function (e) {
        ...
        if (is_email_send == false) { // 발송이 실패하면
            ...
            $.ajax({
                url: `${contextPath}/api/member/emailAuth`, //              /member/ 까진 똑같아서 제외함
...
function goPasswdUpdate() {
    ...
    // 발송 서브밋
    frm.action = `${contextPath}/member/passwdUpdate`;
    frm.method = "post";
    frm.submit();
